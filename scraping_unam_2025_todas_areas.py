"""
Extraccion completa de resultados UNAM 2025 por area.
...
Ejecucion:
    py scraping_unam_2025_todas_areas.py
"""
...
def construir_url_area(area: int) -> str:
    ...
    return (
        ...
        f"Licenciatura{ANIO}/resultados/{area}5.html"
    )


PAGINA_INICIAL_URL = construir_url_area(AREAS[0])

BASE_SALIDA = Path.cwd() / f"resultados_unam_{ANIO}_areas_1_a_4"
...
ARCHIVO_CSV = BASE_SALIDA / f"resultados_unam_{ANIO}_todas_las_areas.csv"
ARCHIVO_EXCEL = BASE_SALIDA / f"resultados_unam_{ANIO}_todas_las_areas.xlsx"
...
def descubrir_opciones(
    ...
    for enlace in enlaces_brutos:
        ...
        rf"/Licenciatura{ANIO}/resultados/(\d+)/(\d+)\.html",
            partes.path,
            flags=re.IGNORECASE,
        )
        if not coincidencia:
...
def cargar_resultado_reutilizable(
    ...
    ruta_anterior = (
        Path.cwd()
        / f"resultados_unam_{ANIO}_area{item['Area']}"
        / "por_opcion"
        / ruta_destino.name
    )
...
def escribir_excel(
    ...
    resumen_por_area = (
        resumen_numerico.groupby(["Anio", "Area"], as_index=False)
        .agg(
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            print("\n" + "=" * 66)
            print(f"CATALOGO GENERAL UNAM {ANIO} - AREAS 1 A 4")
    ...
    df_maestro = df_maestro.sort_values(
        ["Anio", "Area", "Clave_carrera", "Codigo_opcion", "Numero_comprobante"],
        kind="stable",
    ).reset_index(drop=True)
    ...
    print("\n" + "=" * 66)
    print(f"EXTRACCION UNAM {ANIO} FINALIZADA")
